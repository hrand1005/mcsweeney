...
func NewMP4ToMKVEncoder(encodeCapacity int) *MP4ToMKVEncoder {
	...
	go func() {
		...
		for {
			...
			case infile := <-inputFile:
				...
				go encode(infile, outfile, reportChan)
			case <-done:
...
func encode(mp4File, mkvFile string, reportChan chan<- EncodeReport) {
